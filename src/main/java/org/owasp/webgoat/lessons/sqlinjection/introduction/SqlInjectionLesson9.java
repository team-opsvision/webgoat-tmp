...
public class SqlInjectionLesson9 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQueryIntegrity(String name, String auth_tan) {
    ...
    try (Connection connection = dataSource.getConnection()) {
      ...
      // do injectable query
      java.sql.PreparedStatement statement = connection.prepareStatement("SELECT * FROM employees WHERE last_name = ? AND auth_tan = ?", TYPE_SCROLL_SENSITIVE, CONCUR_UPDATABLE);
      statement.setString(1, name);
      statement.setString(2, auth_tan);
      SqlInjectionLesson8.log(connection, "SELECT * FROM employees WHERE last_name = ? AND auth_tan = ?");
      statement.execute();
